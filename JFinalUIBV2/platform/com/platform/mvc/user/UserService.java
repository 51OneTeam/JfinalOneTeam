...
public class UserService extends BaseService {
	 ...
	 */
	@Before(Tx.class)
	public Map req(User user, String password,String email, UserInfo userInfo) {
		Map backMap=new HashMap();
		backMap.put("url", "/jf/platform/index");
		backMap.put("info", "/jf/platform/index");
		...
		try {
			...
			user.set(User.column_password, encryptedPassword);
			//验证邮箱是否可用
			boolean Eamilbool = LoginService.service.valiMailBox("", email);
			//验证手机号是否可用
		//	boolean phoneBool = LoginService.service.valiMobile("", phone);
			if (Eamilbool) {
				userInfo.set(UserInfo.column_email, email);
			}else{
				backMap.put("error", "邮箱已被注册！");
				return backMap;
			}
			
//			if (phoneBool) {
//				userInfo.set(UserInfo.column_mobile, phone);
//			}else{
//				backMap.put("error", "手机号已被注册！");
//				return backMap;
//			}
